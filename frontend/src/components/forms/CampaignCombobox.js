...
/**
 * Searchable campaign field: pick an existing campaign or type a new name.
 */
export default function CampaignCombobox({
  ...
  const exactMatch = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return null;
    return options.find((c) => String(c.label || '').toLowerCase() === q) || null;
  }, [options, query]);

  const showCreateOption = query.trim() && !exactMatch;

  const emit = (campaign_id, campaign_name) => {
    onChange?.({ campaign_id: campaign_id || '', campaign_name: campaign_name || '' });
  };
        ...
        <input
          ...
          onChange={(e) => {
            ...
            const match = options.find(
              (c) => String(c.label || '').toLowerCase() === next.trim().toLowerCase(),
            );
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === 'Enter') {
              ...
              else if (exactMatch) selectOption(exactMatch);
              else {
                commitTyped();
                setOpen(false);
              }
      ...
      {open && !disabled && (
        ...
        >
          {filtered.length === 0 && !showCreateOption && (
            <p className="px-3 py-2 text-xs text-zoho-muted">No campaigns found</p>
          )}
          {filtered.map((c) => (
            <button
              key={c.value}
              ...
              className={`w-full text-left px-3 py-2 text-sm hover:bg-brand-50 transition-colors ${
                c.value === valueId ? 'bg-brand-50 text-brand-700 font-medium' : 'text-zoho-text'
              }`}
          ...
          {showCreateOption && (
            ...
            >
              Use “{query.trim()}” as new campaign
            </button>
          )}
        </div>
      )}
