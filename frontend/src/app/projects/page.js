...
export default function ProjectsPage() {
  ...
  const save = () => {
    showToast('Projects is not available on the Sales CRM API yet');
  };

  return (
    <CRMLayout>
      <div className="p-6">
        <ApiPendingBanner module="Projects" />
      ...
      <ConfirmDialog open={!!deleteTarget} message={`Delete project "${deleteTarget?.name}"?`} confirmLabel="Confirm Delete" danger
        onConfirm={() => { setDeleteTarget(null); showToast('Projects is not available on the Sales CRM API yet'); }} onCancel={() => setDeleteTarget(null)} />
    </CRMLayout>
  );
}
